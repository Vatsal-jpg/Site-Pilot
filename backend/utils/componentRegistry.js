...
// Load directly based on relative path from backend execution context
const componentsData = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'data', 'components.json'), 'utf8'));
const pageRulesData = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'data', 'pageRules.json'), 'utf8'));

// ─── Registry ─────────────────────────────────────────────────────────────────

const components = componentsData;
const pageRules = pageRulesData;
...
/**
 * Get a single component definition by name, returning null if not found instead of throwing.
 */
export function getComponentSafe(name) {
 ...
 */
export function validateSectionList(sections) {
    const errors = [];
 ...
 */
export function getPageDefaults(pageType) {
 ...
 */
export function getBusinessPages(businessType) {
